...
def get_pixel_dist(pixel, red, green, blue):
    ...
    """
    color_distance = ((pixel.red - red)**2 + (pixel.blue - blue)**2 + (pixel.green - green)**2)**(1/2)
...
def get_best_pixel(pixels):
    ...
    # 設定初始值來存取pixel
    for pixel in pixels:
        pixeldis = get_pixel_dist(pixel, avg[0], avg[1], avg[2])
        if pixeldis < min_pixel_dis:
            min_pixel_dis = pixeldis
            min_pixel = pixel
    return min_pixel
